...
public class ModifiableBullet extends Projectile implements ToolProjectile, ProjectileWithPower, ProjectileWithKnockback {

	/** Key to sync the stack to the client */
	protected static final EntityDataAccessor<ItemStack> STACK = SynchedEntityData.defineId(ModifiableBullet.class, EntityDataSerializers.ITEM_STACK);
	/** Movement speed in water */
	protected static final EntityDataAccessor<Float> WATER_INERTIA = SynchedEntityData.defineId(ModifiableBullet.class, EntityDataSerializers.FLOAT);
	...
	public ModifiableBullet(EntityType<? extends ModifiableBullet> type, Level level) {
		super(type, level);
	}

	public ModifiableBullet(Level level, double pX, double pY, double pZ) {
		this(JOTEntities.bullet.get(), level);
		this.setPos(pX, pY, pZ);
	}

	public ModifiableBullet(Level level, LivingEntity shooter) {
	...
	protected void onHitEntity(EntityHitResult result) {
		...
		if (!level.isClientSide) {
			if ((reclaim) && !this.isRemoved()) { //!hit || reclaim
	...
	protected void onHitBlock(@NotNull BlockHitResult result) {
		Vec3i norm = result.getDirection().getNormal();
...
//		JoyOfTinkering.LOGGER.info("bullet hit bloc");
		super.onHitBlock(result);
		if (!this.isRemoved()) {
			if (reclaim) this.spawnAtLocation(stack.copy()); //reclaim lets you keep the gunpowder, but otherwise no dice
			this.discard();
		}
	}
