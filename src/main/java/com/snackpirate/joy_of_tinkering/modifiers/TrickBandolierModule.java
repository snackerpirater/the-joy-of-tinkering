...
public enum TrickBandolierModule implements ModifierModule, BowAmmoModifierHook, GeneralInteractionModifierHook, ModifierRemovalHook, InventorySelectionModule {
	INSTANCE;
	public static final SingletonLoader<TrickBandolierModule> LOADER = new SingletonLoader<>(INSTANCE);
	private static final List<ModuleHook<?>> DEFAULT_HOOKS = HookProvider.<TrickBandolierModule>defaultHooks(ModifierHooks.BOW_AMMO, ModifierHooks.GENERAL_INTERACT, ModifierHooks.REMOVE);
	/** Key for the currently selected arrow */
	private static final ResourceLocation SELECTED_SLOT = JoyOfTinkering.id("trick_bandolier_selected");
	/** Message when disabling the trick quiver */
	private static final Component DISABLED = Component.translatable("modifier.joy_of_tinkering.trick_bandolier.disabled");
	/** Message to display selected slot */
	private static final String SELECTED = "modifier.joy_of_tinkering.trick_bandolier.selected";

	@Override
	public RecordLoadable<TrickBandolierModule> getLoader() {
		return LOADER;
	}
